...
<body class="flex items-center justify-center min-h-screen p-4">
    ...
    <span class="text-white text-3xl font-black tracking-tighter">V</span>
        ...
        <div class="bg-[#0a0a0a] border border-white/[0.08] rounded-2xl p-8 shadow-2xl">
            @if($errors->any())
                <div class="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-xl mb-6 text-sm flex items-center gap-2">
                    <i data-lucide="alert-circle" class="w-4 h-4 shrink-0"></i> {{ $errors->first('email') }}
                </div>
            @endif
            @if(session('success'))
                <div class="bg-blue-500/10 border border-blue-500/20 text-blue-400 px-4 py-3 rounded-xl mb-6 text-sm">{{ session('success') }}</div>
            @endif
        ...
        <p class="text-gray-500 text-sm mt-6 text-center">Belum punya akun? <a href="{{ route('register') }}" class="text-blue-400 hover:text-blue-300 font-semibold hover:underline">Daftar sekarang</a></p>
    </div>
</body>
</html>