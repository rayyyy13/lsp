...
@section('sidebar')
    <div class="p-5">
        ...
        <div class="space-y-1">
            @forelse($hashtags as $tag => $count)
                <a href="/hashtag/{{ $tag }}" class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors group cursor-pointer">
                    <div>
                        <p class="font-bold text-white group-hover:text-blue-400 transition-colors">#{{ $tag }}</p>
                        <p class="text-xs text-gray-600 mt-0.5">{{ $count }} postingan</p>
                    </div>
